...
export const dashboardService = {
  ...
  getAIUsage: () => api.get<{ claude_calls: number; openai_calls: number; deepseek_calls: number; total_tokens: number }>('/dashboard/ai-usage').then((r) => r.data),

  getActivityTimeline: () => api.get<Array<{ type: string; id: string; title: string; time: string; status?: string; channel?: string }>>('/dashboard/activity-timeline').then((r) => r.data),
